...
- Virtualized DOM patch: added an in-memory seen prompt cache per route so prompts already mounted once remain in the timeline even if ChatGPT later unmounts them; the cache is not persisted and does not change V3 storage.
