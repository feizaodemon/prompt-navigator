...
assert(content.includes('handlePromptClick(prompt.id, "compact")'), "compact 圆点点击应使用统一入口精准跳转");
assert(content.includes("COMPACT_SCROLL_OFFSET_PX"), "content.js 应定义 compact 跳转顶部偏移");
assert(content.includes("getBoundingClientRect().top + window.scrollY - COMPACT_SCROLL_OFFSET_PX"), "compact 跳转应使用固定 offset 计算目标位置");
assert(content.includes("window.scrollTo({"), "compact 跳转应滚动到精确目标位置");
